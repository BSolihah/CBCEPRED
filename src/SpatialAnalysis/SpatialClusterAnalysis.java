...
public class SpatialClusterAnalysis {
  ...
  private double distanceBetweenPoint3D(Atom pa, Atom pb)
  {
    ...
    public static ClassificationDataSet prepareDataSet(String filepath, int mode){
        //File file = new File("src/dataset/andersen_exc_3H42_A0.01.arff");  
        System.out.println("filepath:"+ filepath);
        File file = new File(filepath);  
        ClassificationDataSet dataSet = jsat.io.ARFFLoader.loadArffFile(file).asClassificationDataSet(0);
        int num_fitur = dataSet.getNumFeatures();
        for (int i =0;i<1;i++)
            System.out.println(dataSet.getNumericName(i));
        int num_class = dataSet.getClassSize();
        System.out.println("num_fitur:"+ num_fitur + " num_class: "+ num_class);
          
        List<DataPoint> listdata0 = dataSet.getDataPoints();
          /*
          for(int i=0; i<100;i++)
        ...
        ClassificationDataSet dataset_majority = cds_update.getMajorityClassSample();
          ...
          CluSMOTE clusmote = new CluSMOTE();
          if(mode == 0){
              new_cds = clusmote.clusmoteSampling(cds_update, 2);
              System.out.println("datasetawal:"+ cds_update.size());
              System.out.println("datasetbaru:"+ new_cds.size());
          }else if(mode ==1){
            //ClassificationDataSet cds_wnc = clusmote.clusmoteSamplingWithNonClusteredData(cds_update, 2);
            new_cds = clusmote.clusmoteSamplingWithNonClusteredData(cds_update, 2,30);
            
            System.out.println("datasetbaru wnc:"+ new_cds.size());
          ...
          /*
          List<Integer> idx_maj = getIdxFromMajorityClass(dataSet);
          
          List<Integer> idx_min = getIdxFromMinorityClass(dataSet);
          
          System.out.println("jumlah sample: "+ idx_maj.size() +":"+ idx_min.size());
          */
     }
    public void classifyAllEOC(){
        String filetrain = "src/dataset/analysis/andersen_all_0.01.arff";
        ...
        CluSMOTEBagDTModel bag = new CluSMOTEBagDTModel(classifier);
        bag.train(dataSet);
